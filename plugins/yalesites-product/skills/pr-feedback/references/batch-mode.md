...
**Siblings share the outcome but not the labels.** The approval label differs by repo,
`pass functional review` for `yalesites-project`, `atomic`, and `tokens`, `pass design review`
for `component-library-twig`. A unit spanning both repos needs both labels, each on the right
...
the user can do about them, any follow-up tickets offered or created, and anything still waiting
on a decision. **Say explicitly if the batch ended partially done.** A batch that quietly reports
...
2. **The wrong repo's pass label**, because the previous unit's label set got reused.
