...
**Reference Card vs. Spotlight:** For featuring an existing node (a Person profile, a Post, an Event), a **Reference Card** is usually better — it pulls the node's title, image, and metadata automatically. Use Spotlights when you want full editorial control over custom copy and layout for a bespoke feature.
...
Dynamically pulls content from an existing node and displays it as a styled card. Best for featuring an existing Person, Post, Event, Page, or Resource without re-entering content.
...
**Implementation note:** This block's search-as-you-type and month-navigation refresh are driven by a bespoke JS + custom `/events-calendar` AJAX controller — not the standard Views exposed-filter AJAX pipeline used by the other View block modules. See `references/views-reference.md` → Exposed Filters for the accessibility implication.
